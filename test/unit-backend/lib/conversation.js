    ...
    it('should call Conversation.find with the correct "unread" parameter', function(done) {
      ...
      require('../../../backend/lib/conversation')(dependencies, lib).find(options, () => {
        expect(modelsMock.ChatConversation.find).to.have.been.calledWith({
          ...
          $where: `this.numOfMessage > 0 && (!this.memberStates || !this.memberStates["${memberId1}"] || !isNumber(this.memberStates["${memberId1}"].numOfReadMessages) || this.memberStates["${memberId1}"].numOfReadMessages < this.numOfMessage) && (!this.memberStates || !this.memberStates["${memberId2}"] || !isNumber(this.memberStates["${memberId2}"].numOfReadMessages) || this.memberStates["${memberId2}"].numOfReadMessages < this.numOfMessage)`
        });
        done();
      });
    });
